...
interface SQLiteDatabaseSync {
   ...
   * @param maxSize - Maximum number of cached prepared statements before the least-recently-used entry is evicted (default `1000`).
   ...
   * @param entryPoint - Name of the C initialization function to call; when omitted (`null`), SQLite derives it from the filename.
   * @throws {DATABASE_NOT_OPEN} the database is not open.
   * @throws {LOAD_EXTENSION_DISABLED} `allowExtension` was not enabled at construction.
   */
