...
module.exports = class SQLiteStatementSync {
  ...
  all(...params) {
    try {
      const rows = binding.all(this._handle, ...splitParameters(params))
  ...
  get(...params) {
    try {
      const row = binding.get(this._handle, ...splitParameters(params))
  ...
  run(...params) {
    try {
      return binding.run(this._handle, ...splitParameters(params))
    } catch (err) {
      throw errors.from(err)
    }
  }
